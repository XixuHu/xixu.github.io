...
Eigenvector centrality: $C_i=a\sum_{\text{j}}g_{ij}C_j$ , $g_{ij}=\text{I}(\text{j: friend of i})$                 (A self-referential concept)
...
Bonacich centrality: $C^{b}(g)=a g\textbf{1}+b g a g\textbf{1}+b^{2} g^{2} a g\textbf{1}+ \ldots$        
  ...
  give each node a base value $a\cdot d_i(g)$ (const times degree, $a>0$ )             

  add in all paths of length 1 from i to some j times $b$ times j’s base value               

  add in all walks of length 2 from i to some j times $b^2$ times j’s base value                  
